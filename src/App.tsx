...
export default function App() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Spotlight 1: Violet glow in the middle-left
      const spot1X = canvas.width * 0.3 + Math.cos(lightPhase) * 120;
      const spot1Y = canvas.height * 0.4 + Math.sin(lightPhase * 0.8) * 80;
      const spot1Rad = Math.max(1, Math.min(canvas.width, canvas.height) * 0.6);
      if (spot1Rad > 0) {
        const spot1 = ctx.createRadialGradient(spot1X, spot1Y, 0, spot1X, spot1Y, spot1Rad);
        spot1.addColorStop(0, "rgba(168, 85, 247, 0.035)");
        spot1.addColorStop(0.5, "rgba(168, 85, 247, 0.01)");
      ...
      const spot2Rad = Math.max(1, Math.min(canvas.width, canvas.height) * 0.7);
      if (spot2Rad > 0) {
