...
export default function DreamBuilderView({ onDreamCompiled }: DreamBuilderViewProps) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[1400px] mx-auto select-none" id="dream-builder-wrapper">
      ...
      {/* Title HUD block */}
      <header className="glass-panel px-6 py-4 rounded-xl border border-purple-500/10 flex items-center justify-between" id="builder-header">
        <div className="flex items-center gap-3" id="builder-header-title">
          <Wand2 className="w-5 h-5 text-purple-400" />
          <div>
            <h2 className="font-display font-medium text-sm tracking-widest text-purple-200">NEURAL DREAM COMPILER</h2>
            <p className="text-[10px] font-mono text-purple-400/50">CORTICAL VOLUMETRIC SEQUENCE SYNTHESIZER</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" id="builder-grid-container">
        ...
        <section className="lg:col-span-5 glass-panel-heavy p-6 flex flex-col justify-between" id="synthesis-panel">
          <div className="border-b border-purple-500/10 pb-3 mb-4" id="synthesis-header">
            <h3 className="font-display font-medium text-xs tracking-widest text-purple-300">DREAM SOURCE MATERIAL</h3>
            <p className="text-[9px] font-mono text-purple-400/50 mt-0.5">FEED SUB-CONSCIOUS VARIABLES TO CHIP COMPILER</p>
          </div>
          ...
          <form onSubmit={handleCompileDream} className="space-y-4 flex-1" id="synthesis-form">
            ...
            <div className="space-y-1.5" id="synthesis-theme-input">
              <label className="text-[10px] font-mono text-purple-300 uppercase tracking-wider block">1. SYNAPTIC FOCUS ELEMENT (DREAM CORE THEME)</label>
              <textarea
                value={theme}
                ...
                required
                className="w-full h-24 p-3.5 glass-input text-xs text-purple-200 placeholder-purple-400/30 resize-none font-sans leading-relaxed"
            ...
            <div className="space-y-1.5" id="synthesis-backdrop-dropdown">
              <label className="text-[10px] font-mono text-purple-300 uppercase tracking-wider block">2. CORTICAL VOLUMETRIC BACKDROP (SETTING)</label>
              <select
                value={setting}
                onChange={(e) => setSetting(e.target.value)}
                disabled={isCompiling}
                className="w-full p-3 glass-input text-xs text-purple-200"
                id="form-setting-select"
              >
                {settingsList.map((set, idx) => (
                  <option key={idx} value={set} className="bg-[#0b1022] text-purple-200">
                    {set}
                  </option>
                ))}
              </select>
            ...
            <div className="space-y-1.5" id="synthesis-companion-dropdown">
              <label className="text-[10px] font-mono text-purple-300 uppercase tracking-wider block">3. VOLUMETRIC MATRIX CO-ANCHOR (COMPANION)</label>
              <select
                value={companion}
              ...
              >
                {companionsList.map((comp, idx) => (
                  <option key={idx} value={comp} className="bg-[#0b1022] text-purple-200">
                    {comp}
                  </option>
                ))}
              </select>
            ...
            <div className="space-y-1.5" id="synthesis-mood-selector">
              <label className="text-[10px] font-mono text-purple-300 uppercase tracking-wider block">4. EMOTIONAL INTENSITY POLARIZATION (MOOD)</label>
                ...
                {["lucid", "peaceful", "nightmare"].map((m) => (
                  <button
                    ...
                    className={`py-2 px-2.5 rounded border text-[10px] font-display font-medium tracking-widest transition-all duration-300 uppercase ${
                      mood === m
                        ? m === "nightmare"
                          ? "bg-rose-500/20 border-rose-400 text-rose-300 shadow-[0_0_10px_rgba(244,63,94,0.2)]"
                          : "bg-purple-500/25 border-cyan-500/45 text-neon-cyan shadow-[0_0_10px_rgba(168,85,247,0.2)]"
                        : "border-purple-500/10 text-purple-400/60 hover:text-purple-300"
                    }`}
                    id={`form-mood-btn-${m}`}
                  >
                    {m}
        ...
        <section className="lg:col-span-7 glass-panel-heavy p-6 flex flex-col justify-between min-h-[480px]" id="compiled-display-panel">
          
          <div className="border-b border-purple-500/10 pb-3 mb-4" id="compiled-panel-header">
            <h3 className="font-display font-medium text-xs tracking-widest text-purple-300">COMPILED DREAM PROTOCOL</h3>
            <p className="text-[9px] font-mono text-purple-400/50 mt-0.5">SYNAPTIC INLAY RENDER PROTOCOLS</p>
          </div>

          <div className="flex-1 flex flex-col justify-center" id="compiled-panel-body">
            {isCompiling ? (
              /* Live Scrolling Hacking-like logs terminal */
              <div className="bg-black/40 border border-purple-500/10 p-5 rounded-lg h-full min-h-[340px] flex flex-col justify-between font-mono text-[10px] leading-relaxed" id="compiling-logs-box">
                <div className="flex items-center justify-between border-b border-purple-500/10 pb-2 mb-3" id="logs-box-header">
                  ...
                  <span className="text-purple-400/50">NOX_COMPILER v1.2</span>
                </div>
                ...
                <div className="text-center pt-4 border-t border-purple-500/5" id="logs-box-footer">
                  <span className="text-[9px] text-purple-400/30 uppercase tracking-widest block">COUPLING SYNAPSES EN-ROUTE...</span>
                </div>
              </div>
            ) : compiledDream ? (
              ...
              <div className="space-y-4 h-full" id="compiled-readout-box">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between border border-purple-500/15 bg-purple-950/20 p-4 rounded-lg gap-4 text-xs select-text" id="readout-summary">
                  <div id="summary-meta">
                    <span className="text-[8px] font-mono text-purple-400/50 uppercase">COMPILED PROTOCOL:</span>
                    <span className="block font-display font-bold text-neon-purple tracking-widest text-sm uppercase mt-0.5">
                      {compiledDream.protocolName}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-4 text-left font-mono" id="summary-metrics">
                    <div id="summary-freq">
                      <span className="block text-[8px] text-purple-400/50 uppercase">RECOMMENDED FREQ</span>
                      <span className="text-cyan-300 font-bold">{compiledDream.frequency}</span>
                    </div>
                    <div id="summary-stability">
                      <span className="block text-[8px] text-purple-400/50 uppercase">STABILITY</span>
                ...
                <div className="space-y-1" id="readout-narrative-box">
                  <span className="text-[8px] font-mono text-purple-400/55 uppercase tracking-wider block">manifested dream narrative script</span>
                  <div className="max-h-52 overflow-y-auto font-sans text-xs text-purple-200/90 leading-relaxed bg-purple-950/25 border border-purple-500/5 p-4 rounded-lg space-y-3 select-text select-all">
                ...
                <div className="space-y-1.5" id="readout-components-box">
                  <span className="text-[8px] font-mono text-purple-400/55 uppercase tracking-wider block">synthesized somatic render components</span>
                  <div className="grid grid-cols-2 gap-2 text-[10px] font-mono" id="readout-components-grid">
                    {compiledDream.components.map((comp, idx) => (
                      <div key={idx} className="p-2.5 rounded-lg border border-purple-500/10 bg-purple-950/10 text-cyan-300 tracking-wide" id={`readout-comp-${idx}`}>
                        [✓] {comp}
                      </div>
                    ))}
                  </div>
                ...
                {/* Risk profile bar */}
                <div className="flex justify-between items-center text-[10px] font-mono border-t border-purple-500/10 pt-3" id="readout-footer">
                  <span className="text-purple-400/60 uppercase">NEURAL RISK LEVEL:</span>
              ...
              <div className="text-center p-8 border border-purple-500/10 bg-purple-950/10 rounded-xl space-y-4 h-full min-h-[340px] flex flex-col justify-center items-center" id="compiled-uncompiled-placeholder">
                <div className="p-3 bg-purple-500/10 border border-purple-500/25 rounded-full text-purple-400 glow-pulse" id="placeholder-icon-box">
                  <Wand2 className="w-8 h-8" />
                </div>
                <div>
                  <h4 className="font-display font-bold text-xs tracking-widest text-purple-300">COMPILATION SYSTEM READY</h4>
                  <p className="text-xs font-sans text-purple-400/60 mt-1 max-w-sm mx-auto leading-relaxed">
