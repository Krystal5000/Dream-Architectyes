  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[1400px] mx-auto select-none" id="dashboard-view-wrapper">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" id="dashboard-bottom-row">
        ...
        {/* MEMORY REPAIR QUEUE (8 cols) - Highest Elevation */}
        <div className="lg:col-span-8 flex flex-col" id="memory-queue-glow-container-col">
          <div className="relative group flex-1 flex flex-col" id="memory-queue-glow-container">
            {/* Ambient purple/cyan underlying glow */}
            <div className="absolute -inset-1.5 bg-gradient-to-r from-purple-500/10 via-cyan-500/10 to-indigo-500/15 rounded-[24px] blur-xl opacity-80 group-hover:opacity-100 transition duration-500" />
            
            <section className="relative glass-panel-heavy p-6 flex flex-col justify-between flex-1 border-cyan-400/25 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.95)]" id="memory-queue-panel">
              <div className="flex items-center justify-between border-b border-purple-500/15 pb-4 mb-4" id="memory-queue-header">
                <div>
                  <h3 className="font-display font-medium text-xs tracking-widest text-cyan-300 uppercase">MEMORY REPAIR QUEUE</h3>
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-6 min-w-[170px]" id={`subject-actions-box-${sub.id}`}>
                      ...
                      <button
                        ...
                        className={`px-3 py-2 rounded-lg font-display text-[10px] tracking-widest flex items-center gap-1.5 border transition-all duration-300 ${
                          sub.progress >= 100
                            ? "text-purple-500/40 border-purple-500/5 bg-transparent cursor-not-allowed"
        ...
        <section className="lg:col-span-4 glass-panel p-6 rounded-xl flex flex-col justify-between" id="protocols-control-panel">
          <div className="border-b border-purple-500/15 pb-4 mb-4" id="protocols-control-header">
            <h3 className="font-display font-medium text-xs tracking-widest text-purple-300 uppercase">ACTIVE PROTOCOLS</h3>
            <p className="text-[10px] font-mono text-purple-400/50 mt-0.5">COCKPIT CRITICAL INTERLOCK SWITCHES</p>
          </div>
            ...
            {protocols.map((proto) => (
              <div 
                ...
                className={`flex items-center justify-between px-4 py-3 rounded-lg border cursor-pointer select-none transition-all duration-300 ${
                  proto.status === "ACTIVE"
                    ? "bg-purple-950/20 border-cyan-500/35 shadow-[0_0_12px_rgba(6,182,212,0.1)] text-neon-cyan"
                    : "bg-purple-950/5 border-purple-500/10 text-purple-400/60 hover:border-purple-500/25"
                }`}
              ...
              >
                <div className="flex flex-col" id={`protocol-switch-label-${proto.id}`}>
                  ...
                  <span className="text-[8px] font-mono text-purple-400/50 uppercase mt-0.5">{proto.description}</span>
                </div>
                ...
                <div className="flex items-center gap-2.5" id={`protocol-switch-light-${proto.id}`}>
                  ...
                  <div className={`w-3.5 h-3.5 rounded-full border transition-all duration-300 ${
                    proto.status === "ACTIVE"
                      ...
                      : "bg-purple-950 border-purple-500/20"
      ...
      <section className="glass-panel p-4 rounded-xl font-mono text-[10px] text-purple-400/70 border border-purple-500/10" id="cockpit-logs-terminal">
        <div className="flex items-center justify-between border-b border-purple-500/10 pb-2 mb-2" id="logs-terminal-header">
          <span className="text-[9px] tracking-widest font-semibold text-purple-300 flex items-center gap-1.5">
