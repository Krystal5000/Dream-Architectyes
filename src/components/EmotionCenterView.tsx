...
export default function EmotionCenterView({ emotions, setEmotions }: EmotionCenterViewProps) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[1400px] mx-auto select-none" id="emotion-center-wrapper">
      ...
      {/* Title HUD block */}
      <header className="glass-panel px-6 py-4 rounded-xl border border-purple-500/10 flex items-center justify-between" id="emotion-header">
        <div className="flex items-center gap-3" id="emotion-header-title">
          <Activity className="w-5 h-5 text-purple-400" />
          <div>
            <h2 className="font-display font-medium text-sm tracking-widest text-purple-200">NEUROMODULATOR FLUID INJECTORS</h2>
            <p className="text-[10px] font-mono text-purple-400/50">BIO-CHEMICAL TRANSMITTER INTERFACE</p>
          </div>
        </div>
      ...
      <section className="glass-panel p-5 rounded-xl border border-purple-500/10" id="presets-panel">
        <span className="text-[9px] font-mono text-purple-400/50 block uppercase mb-3">SYNAPTIC INJECTOR COCKTAIL PRESETS</span>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4" id="presets-grid">
          ...
          <button
            ...
            disabled={injecting}
            className="px-4 py-3 border border-rose-500/20 bg-rose-950/5 hover:bg-rose-500/15 text-rose-300 font-display text-[10px] tracking-widest rounded-lg transition-all duration-300 uppercase"
          ...
          <button
            ...
            disabled={injecting}
            className="px-4 py-3 border border-indigo-500/20 bg-indigo-950/5 hover:bg-indigo-500/15 text-indigo-300 font-display text-[10px] tracking-widest rounded-lg transition-all duration-300 uppercase"
            id="preset-deep-btn"
          >
            DEEP COMA ALPHA
          </button>
          <button
            ...
            disabled={injecting}
            className="px-4 py-3 border border-purple-500/20 bg-purple-950/5 hover:bg-purple-500/15 text-purple-300 font-display text-[10px] tracking-widest rounded-lg transition-all duration-300 uppercase"
            id="preset-zen-btn"
          >
            ZEN THETA CALM
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" id="emotion-grid-container">
        ...
        <section className="lg:col-span-7 glass-panel-heavy p-6 flex flex-col justify-between" id="injectors-panel">
          <div className="border-b border-purple-500/10 pb-3 mb-4" id="injectors-panel-header">
            <h3 className="font-display font-medium text-xs tracking-widest text-purple-300">BIOCHEMICAL REGULATION PANEL</h3>
            <p className="text-[9px] font-mono text-purple-400/50 mt-0.5">FINE-TUNE TRANSMITTER RATIOS BEFORE FLUID DEPLOYMENT</p>
          </div>

          <div className="space-y-5 flex-1 pr-2" id="injectors-sliders-list">
            
            {/* SEROTONIN */}
            <div className="space-y-1.5 p-3 rounded-lg border border-purple-500/10 bg-purple-950/10" id="injector-row-serotonin">
              ...
              <input
                ...
                disabled={injecting}
                className="w-full h-1 bg-purple-950 rounded-lg appearance-none cursor-pointer accent-cyan-400"
                id="input-serotonin"
              />
            </div>

            {/* DOPAMINE */}
            <div className="space-y-1.5 p-3 rounded-lg border border-purple-500/10 bg-purple-950/10" id="injector-row-dopamine">
            ...
            {/* MELATONIN */}
            <div className="space-y-1.5 p-3 rounded-lg border border-purple-500/10 bg-purple-950/10" id="injector-row-melatonin">
            ...
            {/* CORTISOL */}
            <div className="space-y-1.5 p-3 rounded-lg border border-purple-500/10 bg-purple-950/10" id="injector-row-cortisol">
              <div className="flex justify-between items-center text-xs font-mono" id="label-cortisol">
                <span className="text-rose-400 font-semibold flex items-center gap-1.5 uppercase">
                  <Droplet className="w-3.5 h-3.5 text-rose-500" /> CORTISOL INJECTOR (STRESS / FEAR)
                </span>
                <span className="text-rose-400 font-bold">{cortisol}%</span>
              </div>
              <input
                type="range"
                min="0"
                ...
                disabled={injecting}
                className="w-full h-1 bg-purple-950 rounded-lg appearance-none cursor-pointer accent-rose-500"
                id="input-cortisol"
              />
            </div>

            {/* ANGER/EPINEPHRINE */}
            <div className="space-y-1.5 p-3 rounded-lg border border-purple-500/10 bg-purple-950/10" id="injector-row-anger">
              <div className="flex justify-between items-center text-xs font-mono" id="label-anger">
                <span className="text-rose-400 font-semibold flex items-center gap-1.5 uppercase">
                  <Droplet className="w-3.5 h-3.5 text-rose-500" /> EPINEPHRINE INJECTOR (ANGER / PULSE)
                </span>
                <span className="text-rose-400 font-bold">{anger}%</span>
              </div>
              <input
                type="range"
                min="0"
        ...
        <section className="lg:col-span-5 glass-panel-heavy p-6 flex flex-col justify-between" id="transmitter-panel">
          ...
          <div className="border-b border-purple-500/10 pb-3 mb-4" id="transmitter-panel-header">
            <h3 className="font-display font-medium text-xs tracking-widest text-purple-300">BIOCHEM REGULATION DEPLOYMENT</h3>
            <p className="text-[9px] font-mono text-purple-400/50 mt-0.5">SYNERGISTIC SIGNAL INTERLOCK LOCKOUTS</p>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-center gap-6 my-4" id="transmitter-panel-body">
            ...
            {injected ? (
              <div className="p-5 border border-emerald-500/20 bg-emerald-950/10 rounded-xl text-center space-y-2 flex flex-col items-center justify-center animate-bounce" id="transmitter-success-box">
                ...
                <p className="text-[10px] font-mono text-purple-400/60">Somatic bridge updated successfully. Active EEG patterns stabilized.</p>
              </div>
            ) : (
              <div className="p-5 border border-cyan-500/10 bg-cyan-950/5 rounded-xl space-y-3 font-mono text-[9px] text-cyan-300/70" id="transmitter-specs-box">
                ...
                <div className="flex justify-between items-center" id="chem-telemetry-cortisol">
                  ...
                  <span className="text-rose-400 font-bold">{cortisol} pg/ml</span>
                </div>
                <div className="flex justify-between items-center" id="chem-telemetry-anger">
                  <span>EPI (EPINEPHRINE):</span>
                  <span className="text-rose-400 font-bold">{anger} pg/ml</span>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-cyan-500/10 space-y-3" id="emotion-bar-graph-section">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="h-1.5 w-full bg-cyan-950/50 rounded-full overflow-hidden border border-cyan-500/5">
                      <div 
                        className="h-full bg-gradient-to-r from-cyan-500 to-blue-400 rounded-full transition-all duration-500 shadow-[0_0_8px_rgba(6,182,212,0.5)]" 
                  ...
                  {/* Cortisol Bar */}
                  <div className="space-y-1">
                    <div className="flex justify-between text-[8px] font-mono text-rose-400/60">
                      <span>CORTISOL (FEAR)</span>
                      <span>{cortisol}%</span>
                    </div>
                    <div className="h-1.5 w-full bg-rose-950/50 rounded-full overflow-hidden border border-rose-500/5">
                      <div 
                        className="h-full bg-gradient-to-r from-rose-500 to-red-400 rounded-full transition-all duration-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]" 
            ...
            {cortisol > 80 && (
              <div className="p-3 bg-rose-950/20 border border-rose-500/30 text-rose-400 rounded-lg flex items-start gap-2.5 text-[10px] font-mono leading-relaxed" id="cortisol-danger-alert">
                <ShieldAlert className="w-4 h-4 shrink-0 animate-pulse mt-0.5" />
                <div>
                  <span className="font-bold block uppercase text-rose-300">HYPER-STRESS LEVEL WARNING</span>
          ...
          <div id="chemical-cocktail-actions">
            {injecting && (
              <div className="space-y-1.5 mb-4" id="inject-chemical-progress-box">
                <div className="flex justify-between text-[8px] font-mono text-purple-400/60 uppercase">
                  <span>DEPLOYING BIOCHEMICAL DOSAGE FLUIDS</span>
                  <span className="animate-pulse">DEPLOYING...</span>
                </div>
                <div className="h-1 w-full bg-purple-950 rounded-full overflow-hidden border border-purple-500/10">
